...
USER_AGENT = "runseal-release-beta/1.0"


STABLE_RE = re.compile(r"^(\d+)\.(\d+)\.(\d+)$")
CARGO_BETA_RE = re.compile(r"^(\d+\.\d+\.\d+)-beta\.(?:0|[1-9][0-9]*)$")
...
def read_cargo_version() -> str:
    ...
    beta_match = CARGO_BETA_RE.match(version)
...
def next_beta(cargo_version: str) -> tuple[str, int, str, str]:
    public_url = os.environ.get("RUNSEAL_RELEASES_PUBLIC_URL", "").rstrip("/")
    metadata_url = os.environ.get("RUNSEAL_BETA_METADATA_URL")
    if not metadata_url:
        if not public_url:
