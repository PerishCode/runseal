...
for name in RUNSEAL_RELEASES_S3_AK RUNSEAL_RELEASES_S3_SK RUNSEAL_RELEASES_S3_BUCKET RUNSEAL_RELEASES_S3_URL RUNSEAL_RELEASES_PUBLIC_URL RELEASE_CHANNEL RELEASE_VERSION RELEASE_ROOT GITHUB_OUTPUT GITHUB_REPOSITORY GITHUB_SHA GITHUB_RUN_ID GITHUB_RUN_ATTEMPT GITHUB_WORKFLOW; do
  if [ -z "${!name:-}" ]; then
...
metadata = {
    ...
    "artifacts": {
        "linuxX64": artifact("runseal-x86_64-unknown-linux-gnu.tar.gz", "application/gzip"),
        "macArm64": artifact("runseal-aarch64-apple-darwin.tar.gz", "application/gzip"),
        "macX64": artifact("runseal-x86_64-apple-darwin.tar.gz", "application/gzip"),
        "winX64": artifact("runseal-x86_64-pc-windows-msvc.zip", "application/zip"),
