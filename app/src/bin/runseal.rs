...
use runseal::commands::alias::{
    AliasAppendOptions, resolve_profile_for_alias, run_append as run_alias_append,
    run_list as run_alias_list,
};
use runseal::commands::preview::{PreviewOutputMode, run as run_preview};
use runseal::commands::profiles::{
    InitProfileType, ProfilesInitOptions, run_init as run_profiles_init,
    run_status as run_profiles_status,
};
use runseal::commands::self_update::{SelfUpdateOptions, run as run_self_update};
use runseal::commands::skill::{SkillInstallOptions, run_install as run_skill_install};
use runseal::core::app::App;
use runseal::core::config::{
    CliInput, LogFormat as RuntimeLogFormat, OutputMode, RawEnv, RuntimeConfig,
};
use runseal::helpers::{HelperRunOptions, helper_exit_code, run as run_helper};
use runseal::logging::{SessionLog, current_log_file, make_file_writer, prepare_session_log};
use runseal::run;
...
#[command(
    ...
    version,
    about = "Seal the run.",
    after_help = "Docs: https://runseal.ai/"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<()> {
    ...
    tracing::info!(command = %command_slug(&cli), "runseal invocation started");
    ...
    if let Some(command) = cli.subcommand {
        let result = match command {
            ...
            Commands::Helper(args) => run_helper(HelperRunOptions {
...
fn finish_command(result: Result<()>) -> Result<()> {
    match result {
        ...
        Err(error) => {
            if let Some(code) = helper_exit_code(&error) {
                tracing::error!(exit_code = code, error = %error, "runseal invocation failed");
                eprintln!("{error}");
            ...
            tracing::error!(error = %error, "runseal invocation failed");
            if let Some(path) = current_log_file() {
                eprintln!("See log: {}", path.display());
            }
            Err(error)
...
fn run_external_command(tokens: &[String], run_args: &RunArgs) -> Result<()> {
    let alias_name = parse_shortcut_alias_name(tokens)
        .context("unknown command. alias shortcut must use runseal :<name>")?;
...
fn command_slug(cli: &Cli) -> String {
    match &cli.subcommand {
        Some(Commands::Helper(_)) => "helper".to_owned(),
