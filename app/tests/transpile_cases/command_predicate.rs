...
#[test]
fn command_predicate_emits_targets() {
    let fx = fixture(
        r#"
branch=feat/remote
...
"#,
    ...
    assert!(bash.contains("if git checkout \"$branch\"; then"));
    assert!(powershell.contains("& 'git' 'checkout' $branch"));
    assert!(powershell.contains("if ($LASTEXITCODE -eq 0)"));
