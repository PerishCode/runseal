...
fn release_fixture_roundtrip() {
    ...
    assert!(sealir.contains("regex_capture"));
    assert!(sealir.contains("json_not_empty"));
    assert!(sealir.contains("int_add"));
...
fn release_fixture_emits_targets() {
    ...
    assert!(bash.contains("run_id=$(printf '%s' \"$trigger_output\" | sed -nE"));
    assert!(powershell.contains("$trigger_output = & 'gh' 'workflow' 'run' $workflow"));
    assert!(powershell.contains("[regex]::Match($trigger_output"));
    assert!(powershell.contains("ConvertFrom-Json"));
