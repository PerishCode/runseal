...
DEFAULT_MANAGE_HOST = "runseal.perish.uk"
DEFAULT_MANAGE_ORIGIN_HOST = "releases.runseal.perish.uk"
...
def write_template() -> bool:
    ...
    TOKEN_FILE.write_text(
        "\n".join(
            [
                "# Repo-local Cloudflare credentials for runseal support commands.",
...
def manage_rule_definition(config: CloudflareConfig, spec: ManageRuleSpec) -> dict[str, Any]:
    expression = f'(http.host eq "{config.manage_host}" and http.request.uri.path eq "{spec.path}")'
    return {
        "ref": spec.ref,
...
def add_rule(config: CloudflareConfig, zone_id: str, ruleset_id: str, rule: dict[str, Any]) -> dict[str, Any]:
    return api_request(
        config,
        "POST",
